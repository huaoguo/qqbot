...
const QQBot = (function () {
  ...
  QQBot.prototype.get_user = function (uin) {
    const users = this.buddy_info.info.filter(function (item) {
      return item.uin === uin;
    });
    return users.pop();
  };
  ...
  QQBot.prototype.get_user_ingroup = function (uin, gid) {
    const info = this.groupmember_info[gid];
    const users = info.minfo.filter(function (item) {
      return item.uin === uin;
    });
    return users.pop();
  };
  ...
  QQBot.prototype.get_group = function (options) {
    const groups = this.group_info.gnamelist.filter(function (item) {
      for (const key in options) {
        const value = options[key];
        return item[key] === value;
      }
  ...
  QQBot.prototype.get_dgroup = function (options) {
    try {
      const groups = this.dgroup_info.dnamelist.filter(function (item) {
        for (const key in options) {
          const value = options[key];
          return item[key] === value;
        }
  ...
  QQBot.prototype.get_user_in_dgroup = function (uin, did) {
    try {
      const info = this.dgroupmember_info[did];
      const users = (function () {
        const ref = info.mem_info;
        const results = [];
    ...
    return this.api.get_group_list(this.auth, function (ret, e) {
      if (e) {
        ...
        if (ret.retcode === 0) {
          ...
          const info = self.group_info.gnamelist;
          let n = info.length;
          if (n > 0) {
